...
+ (NSDate*)getLastDayByMonth
{
	...
	NSRange range = [cal rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:currentDate];
...
+(NSString *)timestampIntoTime:(NSString *)dataString mode:(NSString *)mode{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
//    [formatter setDateStyle:NSDateFormatterMediumStyle];
//    [formatter setTimeStyle:NSDateFormatterShortStyle];
    if ([CommonUtils objectIsValid:mode]){
        [formatter setDateFormat:@"YYYY-MM-dd"];
    }
    [formatter setDateFormat:mode];
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:dataString.doubleValue];
