...
{
    if ([self.close.chooseButton1.currentTitle isEqualToString:@"结果"]) {
        
        [[Session sharedSession].tpView presentMessageTips:@"请选择结果"];
    }else if ([self.close.chooseButton2.currentTitle isEqualToString:@"原因"]){
    
        [[Session sharedSession].tpView presentMessageTips:@"请选择原因"];

    }else if ([self.close.chooseButton3.currentTitle isEqualToString:@"关闭时间"]) {
